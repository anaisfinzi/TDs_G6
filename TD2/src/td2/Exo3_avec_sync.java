...
public class Exo3_avec_sync extends Thread {
	private static volatile int seuil=100;
	private Compteur compteur;
	private int id;

	public Exo3_avec_sync(Compteur compteur, int id) {
	...
	public static void main(String[] args) {
		...
		int n = 400;
		Exo3_avec_sync[] tabl = new Exo3_avec_sync[n];
		//long start = System.nanoTime();
		for (int i = 0; i < n; i++) {
			tabl[i] = new Exo3_avec_sync(c, i);
			tabl[i].start();
		}
		for (int i = 0; i < n; i++) {
			try {
